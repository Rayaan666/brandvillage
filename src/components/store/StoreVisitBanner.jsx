...
const StoreVisitBanner = ({ content }) => {
  const eyebrow = content?.eyebrow || "VISIT BRAND VILLAGE OUTLET";
  const titleLine1 = content?.titleLine1 || "See It. Try It.";
  const titleLine2 = content?.titleLine2 || "Take It Home.";
  const description = content?.description || "Visit our store to explore current collections, discover new arrivals and receive personalised assistance from our team.";
  const button1Text = content?.button1Text || "Get Directions";
  ...
  const button2Text = content?.button2Text || "Contact the Store";
  const button2Url = content?.button2Url || "https://wa.me/971581234560";
  const image = content?.image || "https://res.cloudinary.com/n185h1km/image/upload/f_auto,q_auto/MFD053771.jpg_yps3xh";
  const imageAlt = content?.imageAlt || "Premium outlet interior with warm lighting";
